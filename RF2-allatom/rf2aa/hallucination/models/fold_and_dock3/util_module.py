...
def get_seqsep_protein_sm(idx, bond_feats, dist_matrix,sm_mask, cyclic=None): #cycic_reses where?
    '''
    ...
    #res_dist, atom_dist = get_res_atom_dist(idx, bond_feats, sm_mask, cyclize=cyclic)
    res_dist, atom_dist = get_res_atom_dist(idx, bond_feats, dist_matrix, sm_mask, cyclize=cyclic)
...
def get_res_atom_dist(idx, bond_feats, dist_matrix, sm_mask, minpos_res=-32, maxpos_res=32, maxpos_atom=8, cyclize=None):
    ...
    #cyclic_reses = torch.tensor(cyclic_reses).to(gpu)
    # protein residue distances
    seqsep = idx[0,None,:] - idx[0,:,None] # (L, L) 

    # fig, ax = plt.subplots(1,2, figsize=(10,5))

    # ax[0].imshow(seqsep.cpu().numpy())
    # ax[0].set_title('residue distance - before cyclic correction')
    ...
    if cyclize is not None:
        ...
        # ax[1].imshow(seqsep.cpu().numpy())
        # ax[1].set_title('residue distance - after cyclic correction')

        # plt.savefig('res_dist_prot.png')
    ...
    res_dist_prot = torch.clamp(seqsep,
